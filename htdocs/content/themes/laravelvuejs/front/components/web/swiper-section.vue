<template>
  <div class="is-flex:column has-background:secondary">

    <section-heading title="آخر أخبار" white/>

    <div class="row">
      <div class="" v-swiper:articlesSwiper="swiper.options">
        <div class="swiper-wrapper articles-list">
          <article v-for="(item, index) in articles" class="swiper-slide article-item has-background:secondary"
                   :key="index">
        ...
        <nuxt-link to="/" class="articles-button:more has-index:10">المزيد من الأخبار</nuxt-link>
      </div>
    </div>
  </div>
</template>
    ...
    data() {
      return {
        ...
        articles: [
          ...
          'التحولات السريعة و التحولات البطيئة لمجموعة كيميائية'
        ]
      }
    }
  }
...
<style lang="stylus" scoped>
  ...
  .articles-button
    ...
    font-size 20px
    cursor pointer
    transition .25s background-color, .25s opacity, .25s color, .25 border-color
